...
public:
    ...
    IBFCell(int i, int h, int c){
        idSum = i;
        hashSum = h;
        count = c;
    }

    void set(int i, int h, int c){
        idSum = i;
        hashSum = h;
        count = c;
    }
...
void encode(unordered_set<int>& numSet, int N,
    ...
    for(int num : numSet) {
        ...
        for(int idx: idxs){
            ...
            //ibf[idx]->addEntry(num, Hc(num, 1123123123));
...
bool isPure(vector<IBFCell*>& diff, hashFuncType Hc, int i, int N){
    return abs(diff[i]->getCount())==1
            ...
            //&& Hc(diff[i]->getIdSum(), 1123123123)==diff[i]->getHashSum();
...
bool decode(vector<IBFCell*>& diff, unordered_set<int>& diff_A_B,
    ...
    while(!pureList.empty()){
        ...
        for(int idx: idxs){
            ...
            //diff[idx]->setHashSum(diff[idx]->getHashSum()^Hc(idSum, 1123123123));
